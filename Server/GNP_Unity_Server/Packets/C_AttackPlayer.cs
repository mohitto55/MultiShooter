...
{
    public class C_AttackPlayer : IPacket, IPacketHandler
    {
        ...
        {
            Program.MyConsoleWriteLine(dist.ToString());
            if (dist > 500)
            {
                return false;
            }
        ...
        public ArraySegment<byte> Write()
        {
            ...
            ArraySegment<byte> segment = new ArraySegment<byte>(bytes,0,4096);
            ushort count = 0;

            // 데이터 직렬화
            count += sizeof(ushort);

            Array.Copy(BitConverter.GetBytes((int)PacketID.S_AttackPlayer), 0, segment.Array, segment.Offset + count, sizeof(int));
            count += sizeof(int);

            Array.Copy(BitConverter.GetBytes(this.playerId), 0, segment.Array, segment.Offset + count, sizeof(ushort));
            count += sizeof(ushort);

            Array.Copy(BitConverter.GetBytes(this.damagedPlayer), 0, segment.Array, segment.Offset + count, sizeof(ushort));
            count += sizeof(ushort);

            Array.Copy(BitConverter.GetBytes(this.damageAmount), 0, segment.Array, segment.Offset + count, sizeof(ushort));
            count += sizeof(ushort);

            Array.Copy(BitConverter.GetBytes(this.dist), 0, segment.Array, segment.Offset + count, sizeof(ushort));
            count += sizeof(ushort);

            Array.Copy(BitConverter.GetBytes(count), 0, segment.Array, segment.Offset, sizeof(ushort)); // 패킷 크기는 마지막에 입력
            segment = new ArraySegment<byte>(bytes, 0, count);
            return segment;
        }
    }
}
